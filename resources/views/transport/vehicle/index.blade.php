                            ...
                            <tbody>
                                @if ($datas->count() > 0)
                                    @foreach ($datas as $key => $data)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $data->driver->name ?? 'NA' }}</td>
                                            <td>{{ $data->driver->email ?? 'NA' }}</td>
                                            <td>{{ $data->driver->country_code . ' ' . $data->driver->mobile }}</td>
                                            <td>{{ strtoupper($data->vehicle_number) }}</td>
                                            ...
                                            <td>
                                                <button type="button" data-data-id="{{ $data->id }}" id="actionBtn"
        ...
        (function($) {
            $.contextMenu({
                ...
                callback: function(key, options) {
                    ...
                    switch (key) {
                        case "edit":
                            window.location.href = "{{ route('transport.vehicle.edit', ['id' => ':id']) }}"
                                .replace(':id', dataId);
                            break;
                        case 'update':
                            updateDriver(dataId);
            ...
            function updateDriver(dataId) {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('transport.vehicle.updateStatus') }}",
                            type: "POST",
                            data: {
                                _token: _token,
                                vehicle: dataId
            ...
            function deleteDriver(dataId) {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        let _token = $('meta[name="csrf-token"]').attr('content');
                        $.ajax({
                            url: "{{ route('transport.vehicle.delete') }}",
                            type: "POST",
                            data: {
                                _token: _token,
                                vehicle: dataId
