            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('user.products.list') }}" type="button"
                            class="btn btn-primary">@lang('lang.Back')</a>
                    </div>
                    <form action="{{ route('user.products.edit', $product->id) }}" method="post"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label for="name">@lang('lang.Name')</label>
                            <input type="text" class="form-control" id="name"
                                value="{{ old('name', $product->name) }}" name="name" placeholder="@lang('lang.Name')">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="4">{{ old('description', $product->desciption) }}</textarea>
                            @error('description')
                                <label id="description-error" class="error mt-2 text-danger"
                                ...
                                <div class="form-group @error('group') has-danger @enderror">
                                    ...
                                    <select class="group_select w-100 @error('group') form-control-danger @enderror"
                                        ...
                                        @foreach ($groups as $group)
                                            <option @if (old('group', $product->group) == $group->id) selected @endif
                                                value="{{ $group->id }}">
                                                {{ $group->group }} </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group @error('unit_type') has-danger @enderror">
                                    ...
                                    <select class="unit_type_select w-100 @error('unit_type') form-control-danger @enderror"
                                        ...
                                        @foreach ($units as $unit)
                                            <option @if (old('unit_type', $product->unit_type) == $unit->id) selected @endif
                                                value="{{ $unit->id }}">
                                                {{ $unit->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" id="weight"
                                        value="{{ old('weight', $product->weight) }}" name="weight" placeholder="weight">
                                    @error('weight')
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" id="price"
                                        value="{{ old('price', $product->price) }}" name="price" placeholder="price">
                                    @error('price')
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" id="tax"
                                        value="{{ old('tax', $product->tax) }}" name="tax" placeholder="tax">
                                    @error('tax')
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" id="stock"
                                        value="{{ old('stock', $product->stock) }}" name="stock" placeholder="stock">
                                    @error('stock')
                        ...
                        <div class="mb-2">
                            <input type="file" name="image"
                                data-default-file="{{ asset($product->image_base . '/' . $product->image) }}"
                                class="dropify" data-max-file-size="2024kb" />
                            @error('image')
                                <label id="image-error" class="error mt-2 text-danger"
                                    for="image">{{ $message }}</label>
                            @enderror
                        </div>
                        <button class="btn mt-3 btn-sm form-control btn-primary">Update Product</button>
    ...
    <script>
        (function($) {
            ...
            function getBrand(group) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('user.products.brands.list') }}",
                    data: {
                        _token: _token,
                        group: group
                    },
                    success: function(response) {
                        if (response.success) {
                            ...
                            let oldValue =
                                '{{ old('old', $product->brand) }}'; // You might need to adjust this
                            response.data.forEach(function(brand) {
                                let option = $('<option>', {
                                    value: brand.id,
                                    text: brand.brand
                                });
                                if (brand.id == oldValue) {
            ...
            @if (old('group', $product->group))
                getBrand('{{ old('group', $product->group) }}')
            @endif
        })(jQuery);
    </script>
@endsection
