            ...
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        @if (request('trash') == 1)
                        <a href="{{ route('user.farmers.list') }}" class="btn btn-primary mr-2">
                            @lang('lang.:name List', ['name' => __('lang.Farmer')])</a>
                        @if ($farmers->count() > 0)
                        <a href="{{ route('user.farmers.deleteAll') }}" class="btn btn-primary">
                            @lang('lang.Delete All')</a>
                        @endif
                        @else
                        <a href="{{ route('user.farmers.list') }}?trash=1" class="btn btn-primary mr-2">
                            @lang('lang.Trash')</a>
                        <a href="{{ route('user.farmers.create') }}" class="btn btn-primary"> @lang('lang.Add :name', ['name' => __('lang.Farmer')])</a>
                        @endif
                    </div>
                </div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" data-farmer-id="{{ $farmer->farmer_id }}"
                            ...
                            @endforeach
                            @if ($farmers->total() > env('PER_PAGE_RECORDS'))
                            <tr>
                                <td colspan="6">
    ...
    (function($) {
        $.contextMenu({
            ...
            callback: function(key, options) {
                var farmer_id = $(this).data('farmer-id');
                var name = $(this).data('farmer-name');
                var m = "clicked: " + key;
                if (key == 'edit') {
                    window.location.href = "{{ route('user.farmers.edit', ['farmer_id' => ':id']) }}"
                        .replace(':id', farmer_id);
                } else if (key == 'view') {
                    window.location.href = "{{ route('user.farmers.view', ['farmer_id' => ':id']) }}"
                        .replace(':id', farmer_id);
                } else if (key == 'delete') {
                    @if(request('trash') == 1)
                    farmerSwalPDelete(name, farmer_id);
                    @else
                    farmerSwal(name, farmer_id);
                    @endif
                }
                @if(request('trash') == 1)
                else if (key == 'restore') {
                    farmerSwalRestore(name, farmer_id);
        ...
        farmerSwalRestore = function(name, farmerId) {
            swal.fire({
                title: '@lang('
                lang.Are you sure ? ')',
                text : "@lang('lang.You want to Restore :name to the farmer list.')".replace(':name', name),
                icon: 'warning',
                timer: 2000,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('user.farmers.restore') }}",
                        type: "POST",
                        data: {
                            _token: _token,
                            farmer_id: farmerId
        ...
        farmerSwalPDelete = function(name, farmerId) {
            swal.fire({
                title: '@lang('
                lang.Are you sure ? ')',
                text : "@lang('lang.You want to permanent delete :name from the farmer list.')".replace(':name', name),
                icon: 'warning',
                timer: 2000,
                confirmButtonText: '@lang('
                lang.Yes ')',
