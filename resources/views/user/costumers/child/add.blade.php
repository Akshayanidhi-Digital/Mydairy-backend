...
@section('content')
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">@lang('lang.Add :name', ['name' => $role_type])</h4>
                    <form id="childDairy" action="{{ route('user.childUser.store', $role_type) }}" method="POST"
                        autocomplete="off">
                        @csrf
                        <div class="row">
                            <div class="col-sm-6">
                                    ...
                                    <select class="role_select w-100" id="role" name="role">
                                        ...
                                        @foreach ($roles as $r)
                                            <option value="{{ $r->role_id }}"
                                                @if (old('role') == $r->role_id) selected
                                           @elseif($r->short_name == $role_type) selected @endif>
                                                {{ $r->short_name . ' - ' . $r->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="dairy_name"
                                        value="{{ old('dairy_name') }}" name="dairy_name" placeholder="@lang('lang.Dairy Name')">
                                    @error('dairy_name')
                        ...
                        <button type="submit" class="btn btn-primary me-2">@lang('lang.Add :name', ['name' => $role_type])</button>
                        <a href="{{ route('user.childUser.list', $role_type) }}"
                            class="btn btn-light">@lang('lang.Back')</a>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        (function($) {
            ...
            jQuery.validator.addMethod("address", function(value, element) {
                return this.optional(element) || /((\d{1,2}\/\d{1,2} [A-Za-z0-9\s]+)?|([A-Za-z0-9\s]+)?)(, [A-Za-z]+)?(, [A-Z]{2})?(, \d{5,6})?$/i.test(value);
                ...
                $("#childDairy").validate({
                    ...
                    messages: {
                        ...
                        address: {
                            ...
                            address: "@lang('lang.name.lettersonly')",
                        },
                        latitude: {
