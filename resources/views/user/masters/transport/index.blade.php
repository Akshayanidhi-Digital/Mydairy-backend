            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex  justify-content-between align-content-center">
                        <h4 class="card-title">@lang('lang.:name List', ['name' => __('lang.Transporter')])</h4>
                        <a href="{{ route('user.masters.transport.add') }}" class="btn btn-primary">@lang('lang.Add :name', ['name' => __('lang.Transporter')])</a>
                    </div>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th>@lang('lang.Transport ID')</th>
                                    <th>@lang('lang.Transport Name')</th>
                                ...
                                @if ($datas->count() > 0)
                                    @foreach ($datas as $key => $data)
                                        <tr>
                                            ...
                                            <td>{{ $data->transporter_id }}</td>
                                            <td>{{ $data->transporter_name }}</td>
                                            <td>
                                                {{ $data->name . ' S/o ' . $data->father_name }}
                                            </td>
                                            <td>
                                            ...
                                            @else
                                                <td><label class="badge badge-primary">@lang('lang.Active')</label></td>
                                            @endif
                                            <td>
                                                <button type="button" data-transport-id="{{ $data->transporter_id }}"
                                                    data-status={{ $data->is_blocked }} id="actionOnRoutes"
        ...
        (function($) {
            ...
            $.contextMenu({
                ...
                callback: function(key, options) {
                    var transporter_id = $(this).data('transport-id');
                    var status = $(this).data('status');
                    if (key == 'edit') {
                        window.location.href =
                            "{{ route('user.masters.transport.edit', ['transporter_id' => ':id']) }}"
                            .replace(
                                ':id',
                                transporter_id);
                    } else if (key == 'status') {
                        swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    url: "{{ route('user.masters.transport.status') }}",
                                    type: "POST",
                                    data: {
                                        _token: _token,
