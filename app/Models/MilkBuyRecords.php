...
class MilkBuyRecords extends Model
{
    ...
    {
        return $this->hasOne(MilkTransportRecords::class, 'record_id', 'id');
    }

    public function scopeWithTransportDetails($query)
    {
                ...
                $subQuery->where('record_type', 1)
                    ->whereExists(function ($existsQuery) {
                        $existsQuery->select(DB::raw(1))
                            ->from('milk_transport_records')
                            ->whereColumn('milk_transport_records.record_id', 'milk_buy_records.id');
                    });
            });
        });
    }
            ...
            $query->where(function ($subQuery) {
                $subQuery->where('record_type', 0)
                    ->whereExists(function ($existsQuery) {
                        $existsQuery->select(DB::raw(1))
                            ->from('farmers')
                            ->whereColumn('farmers.farmer_id', 'milk_buy_records.seller_id');
                    });
            })
                ->orWhere(function ($subQuery) {
                    $subQuery->where('record_type', 1)
                        ->whereExists(function ($existsQuery) {
                            $existsQuery->select(DB::raw(1))
                                ->from('users')
                                ->whereColumn('users.user_id', 'milk_buy_records.seller_id');
                        });
                })
    ...
    protected static function booted()
    {
        static::retrieved(function ($data) {
            ...
            if ($data->record_type == 2) {
                ...
                $costumer = User::where(['user_id' => $data->seller_id])->first();
                $data->costumer = $costumer ?: null;
            } else {
                $costumer = Farmer::where(['farmer_id' => $data->seller_id])->first();
                $data->costumer = $costumer ?: null;
            }
        });
